...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(
        ...
        with torch.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION, device_type='cuda'):

            # Explicitly declare reduction to mean.
            perform_backward = True
            if (cur_iter + 1) % cfg.SOLVER.ACCUMULATE_STEPS == 0 or (cur_iter + 1) == len(train_loader):
                optimizer.zero_grad()
            ...
            if len(train_loader) % cfg.SOLVER.ACCUMULATE_STEPS == 0:
                accumulate_divisor = cfg.SOLVER.ACCUMULATE_STEPS
            else:
                accumulate_divisor = len(train_loader) % cfg.SOLVER.ACCUMULATE_STEPS if (cur_iter + 1) > (((cur_iter + 1) // cfg.SOLVER.ACCUMULATE_STEPS) * cfg.SOLVER.ACCUMULATE_STEPS) else cfg.SOLVER.ACCUMULATE_STEPS
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss:
                loss = partial_loss
            elif isinstance(labels, dict) and cfg.TRAIN.DATASET == "epickitchens":
                # Compute the loss.
                loss_verb = loss_fun(preds[0], labels['verb']) / accumulate_divisor
                loss_noun = loss_fun(preds[1], labels['noun']) / accumulate_divisor
                loss = 0.5 * (loss_verb + loss_noun) 
            else:
                loss = loss_fun(preds, labels) / accumulate_divisor
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if isinstance(labels, (dict,)) and cfg.TRAIN.DATASET == "epickitchens":
                ...
                loss = loss * accumulate_divisor
                loss_verb = loss_verb * accumulate_divisor
                loss_noun = loss_noun * accumulate_divisor
