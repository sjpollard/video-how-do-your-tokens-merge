...
from .timesformer_utils import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from .timesformer_helpers import load_pretrained
from .timesformer_utils import DropPath, to_2tuple, trunc_normal_
...
class TimeSformer(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.model = VisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=self.num_classes, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, **kwargs)

        self.attention_type = cfg.TIMESFORMER.ATTENTION_TYPE
        self.model.default_cfg = default_cfgs['TimeSformer']
...
@MODEL_REGISTRY.register()
class TimeSformerNoCfg(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        super(TimeSformerNoCfg, self).__init__()
        self.pretrained=True
