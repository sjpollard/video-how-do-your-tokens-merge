...
def plot_confusion_matrices(cmtxs, titles, num_classes, class_names=None, figsize=None):
    if class_names is None or type(class_names) != list:
        class_names = [str(i) for i in range(num_classes)]

    fig, axs = plt.subplots(1, len(cmtxs), figsize=figsize, squeeze=False)
    for k, cmtx in enumerate(cmtxs):
        im = axs[0][k].imshow(cmtx, interpolation="nearest", cmap='RdBu', norm=TwoSlopeNorm(0))
        axs[0][k].set_title(titles[k])
        tick_marks = np.arange(len(class_names[k]))
        axs[0][k].set_xticks(tick_marks, class_names[k], rotation=45)
        axs[0][k].set_yticks(tick_marks, class_names[k])
        if k == 0:
            axs[0][k].set_ylabel("True label")
        axs[0][k].set_xlabel("Predicted label")

        # Use white text if squares are dark; otherwise black.
        blue_threshold = cmtx.max() / 2.0
        red_threshold = cmtx.min() / 2.0
        for i, j in itertools.product(range(cmtx.shape[0]), range(cmtx.shape[1])):
            color = "white" if cmtx[i, j] > blue_threshold or cmtx[i, j] < red_threshold else "black"
            axs[0][k].text(
                j,
                i,
                "",
        ...
        divider = make_axes_locatable(axs[0][k])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        cbar = fig.colorbar(im, cax=cax)
        cbar.set_ticks([cmtx.max(), 0, cmtx.min()], labels=[str(round(cmtx.max(), 2)), str(round(0, 2)), str(round(cmtx.min(), 2))])
