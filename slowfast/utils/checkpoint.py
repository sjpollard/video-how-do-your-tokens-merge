...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        # Fix checkpoints if they're not SlowFast style
        if 'model_state' not in checkpoint.keys():
            # Fix VideoMAE checkpoint
            if 'module' in checkpoint.keys():
                checkpoint = {'model_state': checkpoint['module']}
                temp = {f'model.{k}': v for k, v in checkpoint['model_state'].items()}
                checkpoint['model_state'] = temp
            # Fix ViViT checkpoint
            else: 
